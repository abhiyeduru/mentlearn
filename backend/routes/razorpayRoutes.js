...
// Helper: compute discount and validity
const computeCouponDiscount = (price, coupon) => {
  const now = new Date();

  if (!coupon) return { valid: false, reason: 'Coupon not found', discount: 0, finalPrice: price };
  if (coupon.active === false) return { valid: false, reason: 'Coupon is inactive', discount: 0, finalPrice: price };

  if (coupon.startDate) {
    const start = new Date(coupon.startDate);
    if (!isNaN(start.getTime()) && now < start) {
      return { valid: false, reason: 'Coupon not started yet', discount: 0, finalPrice: price };
    }
  }
  ...
  if (coupon.expiryDate) {
    ...
    if (!isNaN(expiry.getTime()) && now > expiry) {
      return { valid: false, reason: 'Coupon expired', discount: 0, finalPrice: price };
    }
  }

  if (coupon.minAmount && Number(coupon.minAmount) > 0 && price < Number(coupon.minAmount)) {
    return { valid: false, reason: `Minimum order amount is ₹${coupon.minAmount}`, discount: 0, finalPrice: price };
  }
  ...
  return { valid: true, reason: 'Coupon applied', discount, finalPrice };
};
...
router.post('/create-order', verifyFirebaseToken, async (req, res) => {
  ...
  try {
    ...
    let subtotal = Number(courseData.price) || 0; // base course price in rupees
    let discountedPrice = subtotal;
    let discount = 0;

    // Apply coupon if provided (using robust validation)
    if (couponCode) {
      const cleanCode = String(couponCode).trim().toUpperCase();
      const couponRef = admin.firestore().collection('coupons').doc(cleanCode);
      const couponDoc = await couponRef.get();
      const coupon = couponDoc.exists ? couponDoc.data() : null;
      const result = computeCouponDiscount(subtotal, coupon);
      if (result.valid) {
    ...
    // GST exclusive: add GST on top of discounted price
    const gstRate = Number(process.env.GST_RATE || process.env.RAZORPAY_GST_RATE || 1.9);
    const gstAmount = +(discountedPrice * (gstRate / 100)).toFixed(2);
    const finalPayable = +(discountedPrice + gstAmount).toFixed(2);
...
// POST /api/razorpay/validate-coupon
// Validate a coupon without creating an order
router.post('/validate-coupon', verifyFirebaseToken, async (req, res) => {
  try {
    const { courseId, couponCode } = req.body;
    if (!courseId || !couponCode) {
      return res.status(400).json({ ok: false, error: 'courseId and couponCode are required' });
    }
    ...
    const courseData = courseDoc.data();
    const price = Number(courseData.price) || 0; // subtotal before GST
    ...
    const result = computeCouponDiscount(price, coupon);
    const discountedPrice = result.valid ? result.finalPrice : price;
    const discount = result.valid ? result.discount : 0;
    const gstRate = Number(process.env.GST_RATE || process.env.RAZORPAY_GST_RATE || 1.9);
    const gstAmount = +(discountedPrice * (gstRate / 100)).toFixed(2);
    const total = +(discountedPrice + gstAmount).toFixed(2);
    ...
    return res.json({
      ...
      gstRate,
      gstAmount,
      message: result.valid ? 'Coupon applied' : (result.reason || 'Invalid coupon')
    });
  } catch (error) {
