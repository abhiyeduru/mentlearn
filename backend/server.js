...
// ===== ROUTES =====
try {
  const liveSessionsRoutes = require('./routes/liveSessionsRoutes');
  app.use('/api/live-sessions', liveSessionsRoutes);
  console.log('✅ Live Sessions routes registered');
} catch (error) {
  console.error('❌ Error loading live sessions routes:', error);
}
...
// Hiring Partner Portal Routes
try {
  const hiringPartnerRoutes = require('./routes/hiringPartnerRoutes');
  app.use('/api/hiring-partners', hiringPartnerRoutes);
  console.log('✅ Hiring Partner routes registered');
} catch (error) {
  console.error('❌ Error loading hiring partner routes:', error);
}

// TODO: Convert these routes to Firestore
/*
try {
  const jobPostingRoutes = require('./routes/jobPostingRoutes');
  app.use('/api/jobs', jobPostingRoutes);
  console.log('✅ Job Posting routes registered');
} catch (error) {
  console.error('❌ Error loading job posting routes:', error);
}

try {
  const jobApplicationRoutes = require('./routes/jobApplicationRoutes');
  app.use('/api/applications', jobApplicationRoutes);
  console.log('✅ Job Application routes registered');
} catch (error) {
  console.error('❌ Error loading job application routes:', error);
}

try {
  const studentDiscoveryRoutes = require('./routes/studentDiscoveryRoutes');
  app.use('/api/students', studentDiscoveryRoutes);
  console.log('✅ Student Discovery routes registered');
} catch (error) {
  console.error('❌ Error loading student discovery routes:', error);
}
*/
