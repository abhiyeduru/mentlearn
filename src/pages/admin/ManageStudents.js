...
export default function ManageStudents() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Fetch students and mentors in parallel
        const [studentDocs, mentorDocs, allEnrollmentDocs, allCourseDocs] = await Promise.all([
        ...
        allEnrollmentDocs.docs.forEach(doc => {
          const data = doc.data();
          if (!enrollmentsByStudent.has(data.studentId)) {
            enrollmentsByStudent.set(data.studentId, []);
          }
          const course = coursesMap.get(data.courseId);
          if (course) {
            enrollmentsByStudent.get(data.studentId).push({
              id: doc.id,
              ...data,
        ...
        // Map students with their enrollments
  ...
  const exportToCSV = () => {
    const headers = ['Name', 'Email', 'Phone', 'Enrolled On', 'Total Enrollments', 'Enrolled Courses'];
