...
const ManageDataAnalysts = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Store the password temporarily to display
      setCreatedPassword(formData.password);
      
      setActionStatus({
        type: 'success',
        message: `Data analyst created successfully! Password: ${formData.password} (Save this - it cannot be retrieved later)`
      });
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {analysts.map((analyst) => (
                    <tr key={analyst.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-500 italic">Encrypted (Firebase)</div>
                        <div className="text-xs text-gray-400">Cannot be retrieved</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
