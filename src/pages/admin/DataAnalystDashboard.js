...
const DataAnalystDashboard = () => {
  ...
  const [liveSessionBookings, setLiveSessionBookings] = useState([]);
  ...
  // Live Sessions Management States
  const [liveSessionTab, setLiveSessionTab] = useState('registrations');
  const [liveSessionRegistrations, setLiveSessionRegistrations] = useState([]);
  const [liveSessionForms, setLiveSessionForms] = useState([]);
  const [lsSearchTerm, setLsSearchTerm] = useState('');
  const [lsFilterStatus, setLsFilterStatus] = useState('all');
  ...
  const fetchLiveSessionRegistrations = async () => {
    try {
      ...
      setLiveSessionRegistrations(data);
    } catch (error) {
      console.error('Error fetching registrations:', error);
    }
  };

  const fetchLiveSessionForms = async () => {
    try {
      const formsRef = collection(db, 'liveSessionForms');
  ...
  const handleCreateSession = async (e) => {
    ...
    try {
      ...
      await fetchLiveSessionForms();
      alert('Session created successfully!');
      setLiveSessionTab('sessions');
    } catch (error) {
  ...
  const updateRegistrationStatus = async (regId, newStatus) => {
    try {
      ...
      await fetchLiveSessionRegistrations();
    } catch (error) {
      console.error('Error updating status:', error);
    }
  };

  const toggleSessionActive = async (sessionId, currentStatus) => {
    try {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      // 1.1 Get live session bookings
      try {
        const liveSessionQuery = query(
          collection(db, "liveSessionBookings"),
          orderBy("timestamp", "desc")
        );
        const liveSessionSnapshot = await getDocs(liveSessionQuery);
        const liveSessionData = liveSessionSnapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data(),
          createdAt: doc.data().timestamp?.toDate ? doc.data().timestamp.toDate() : new Date(doc.data().createdAt)
        }));
        setLiveSessionBookings(liveSessionData);
      } catch (error) {
        console.error('Error fetching live session bookings:', error);
      ...
      // 6. Get advisor requests
      try {
        const advisorQuery = query(
          collection(db, "advisorRequests"),
          orderBy("createdAt", "desc")
        );
        const advisorSnapshot = await getDocs(advisorQuery);
        const advisorData = advisorSnapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data()
        }));
        setAdvisorRequests(advisorData);
      } catch (error) {
        console.error('Error fetching advisor requests:', error);
        setAdvisorRequests([]);
      }

      // 6.1. Get partner requests
      try {
        ...
        setPartnerRequests(partnerData);
      } catch (error) {
        console.error('Error fetching partner requests:', error);
        setPartnerRequests([]);
      }
      
      // 7. Get user activities from multiple potential sources
  ...
  useEffect(() => {
    ...
    // Fetch live session data if on that tab
    if (activeTab === 'livesession') {
      fetchLiveSessionRegistrations();
      fetchLiveSessionForms();
    }
  }, [currentUser, userRole, navigate, dateRange, fetchDashboardData, activeTab]);
          ...
          <div className="py-6 px-3">
            <ul className="space-y-2">
              ...
              <NavLink 
                icon={FaVideo} 
                text="Live Session" 
                isActive={activeTab === 'livesession'} 
              ...
              <NavLink 
                ...
                text="Leads" 
                isActive={false} 
                onClick={() => navigate('/data-analyst/leads')} 
              />
              <NavLink 
                icon={FaLifeRing} 
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
              <div>
                <h1 className="text-2xl font-semibold">
                  ...
                  {activeTab === 'livesession' && 'Live Session Bookings'}
                  ...
                  {activeTab === 'advisor' && 'Advisor Requests'}
                  {activeTab === 'partner' && 'Partner Requests'}
                ...
                {activeTab === 'overview' && (
                  <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
                      <button
                        onClick={() => navigate('/admin/live-sessions-manage')}
                        className="bg-gradient-to-br from-red-600 to-pink-600 p-6 rounded-xl shadow-lg hover:shadow-xl transition-all hover:scale-105"
                      >
                        <div className="flex flex-col items-center text-center">
                          <FaVideo className="h-8 w-8 mb-3" />
                          <h3 className="font-semibold text-sm">Live Sessions</h3>
                          <p className="text-xs mt-1 opacity-80">Manage & View</p>
                        </div>
                      </button>
                      <button
                        onClick={() => navigate('/admin/hiring-partners')}
                        className="bg-gradient-to-br from-purple-600 to-purple-800 p-6 rounded-xl shadow-lg hover:shadow-xl transition-all hover:scale-105"
                      >
                        <div className="flex flex-col items-center text-center">
                          <FaBriefcase className="h-8 w-8 mb-3" />
                          <h3 className="font-semibold text-sm">Hiring Partners</h3>
                        </div>
                      </button>
                      <button
                        onClick={() => navigate('/admin/why-mentneo')}
                        className="bg-gradient-to-br from-blue-600 to-blue-800 p-6 rounded-xl shadow-lg hover:shadow-xl transition-all hover:scale-105"
                      >
                        <div className="flex flex-col items-center text-center">
                          <FaChartLine className="h-8 w-8 mb-3" />
                          <h3 className="font-semibold text-sm">Why Mentneo</h3>
                        </div>
                      </button>
                      <button
                        onClick={() => navigate('/admin/showcase-videos')}
                        className="bg-gradient-to-br from-purple-500 to-indigo-600 p-6 rounded-xl shadow-lg hover:shadow-xl transition-all hover:scale-105"
                      >
                        <div className="flex flex-col items-center text-center">
                          <FaVideo className="h-8 w-8 mb-3" />
                          <h3 className="font-semibold text-sm">Showcase Videos</h3>
                        </div>
                      </button>
                      <button
                        onClick={() => navigate('/admin/landing-videos')}
                        className="bg-gradient-to-br from-pink-600 to-pink-800 p-6 rounded-xl shadow-lg hover:shadow-xl transition-all hover:scale-105"
                      >
                        <div className="flex flex-col items-center text-center">
                          <FaVideo className="h-8 w-8 mb-3" />
                          <h3 className="font-semibold text-sm">Landing Videos</h3>
                        </div>
                      </button>
                      <button
                        onClick={() => navigate('/admin/masterclass-mosaic')}
                        className="bg-gradient-to-br from-indigo-600 to-indigo-800 p-6 rounded-xl shadow-lg hover:shadow-xl transition-all hover:scale-105"
                      >
                        <div className="flex flex-col items-center text-center">
                          <FaChalkboardTeacher className="h-8 w-8 mb-3" />
                          <h3 className="font-semibold text-sm">Masterclass Mosaic</h3>
                        </div>
                      </button>
                      <button
                        onClick={() => navigate('/admin/live-bookings')}
                        className="bg-gradient-to-br from-green-600 to-green-800 p-6 rounded-xl shadow-lg hover:shadow-xl transition-all hover:scale-105"
                      >
                        <div className="flex flex-col items-center text-center">
                          <FaCalendarCheck className="h-8 w-8 mb-3" />
                          <h3 className="font-semibold text-sm">Live Bookings</h3>
                        </div>
                      </button>
                    </div>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                      <KpiCard 
                        ...
                        value={liveSessionRegistrations.length}
                        color="text-purple-500"
                        icon={FaUsers}
                      />
                      <KpiCard 
                        title="Pending" 
                        value={liveSessionRegistrations.filter(r => r.status === 'pending' || !r.status).length}
                        color="text-yellow-500"
                        icon={FaClock}
                      />
                      <KpiCard 
                        title="Active Sessions" 
                        value={liveSessionForms.filter(s => s.isActive).length}
                        color="text-blue-500"
                        icon={FaVideo}
                      />
                      <KpiCard 
                        title="Confirmed" 
                        value={liveSessionRegistrations.filter(r => r.status === 'confirmed').length}
                        color="text-green-500"
                        icon={FaCheckCircle}
                      />
                    </div>
                    ...
                    {/* Sub Tabs */}
                    <div className="bg-[#141426] p-1 rounded-xl flex gap-2 mb-6">
                      <button
                        onClick={() => setLiveSessionTab('registrations')}
                        className={`flex-1 py-3 px-4 rounded-lg font-medium transition-all flex items-center justify-center gap-2 ${
                          liveSessionTab === 'registrations'
                            ? 'bg-purple-600 text-white'
                            : 'text-gray-400 hover:text-white'
                        }`}
                      >
                        <FaUsers /> Registrations ({liveSessionRegistrations.length})
                      </button>
                      <button
                        onClick={() => setLiveSessionTab('sessions')}
                        className={`flex-1 py-3 px-4 rounded-lg font-medium transition-all flex items-center justify-center gap-2 ${
                          liveSessionTab === 'sessions'
                            ? 'bg-purple-600 text-white'
                            : 'text-gray-400 hover:text-white'
                        }`}
                      >
                        <FaVideo /> Sessions ({liveSessionForms.length})
                      </button>
                      <button
                        onClick={() => setLiveSessionTab('create')}
                        className={`flex-1 py-3 px-4 rounded-lg font-medium transition-all flex items-center justify-center gap-2 ${
                          liveSessionTab === 'create'
                            ? 'bg-purple-600 text-white'
                            : 'text-gray-400 hover:text-white'
                        }`}
                      >
                    ...
                    {liveSessionTab === 'registrations' && (
                      <div className="bg-[#141426] p-6 rounded-2xl shadow-lg">
                        <div className="flex justify-between items-center mb-6">
                          <h3 className="text-lg font-medium">All Registrations</h3>
                          <div className="flex gap-4">
                            <input
                              type="text"
                              placeholder="Search..."
                              value={lsSearchTerm}
                              onChange={(e) => setLsSearchTerm(e.target.value)}
                              className="px-4 py-2 bg-[#1c1c38] border border-gray-700 rounded-lg text-sm"
                            />
                            <select
                              value={lsFilterStatus}
                              onChange={(e) => setLsFilterStatus(e.target.value)}
                              className="px-4 py-2 bg-[#1c1c38] border border-gray-700 rounded-lg text-sm"
                            >
                            ...
                            <button
                              onClick={() => {
                                const csvContent = [
                                  ['Date', 'Name', 'Email', 'Phone', 'Session', 'Level', 'Status'].join(','),
                                  ...liveSessionRegistrations.map(r => [
                                    r.registeredAt?.toLocaleDateString?.() || 'N/A',
                                    r.fullName || 'N/A',
                                    r.email || 'N/A',
                                    r.phone || 'N/A',
                                    r.selectedSession || 'N/A',
                                    r.studentLevel || 'N/A',
                                    r.status || 'pending'
                                  ].map(cell => `"${cell}"`).join(','))
                                ].join('\\n');
                          ...
                          <table className="min-w-full divide-y divide-gray-800">
                            ...
                            <tbody className="divide-y divide-gray-800">
                              {liveSessionRegistrations
                                .filter(r => {
                                  const matchesSearch = !lsSearchTerm || 
                                    r.fullName?.toLowerCase().includes(lsSearchTerm.toLowerCase()) ||
                                    r.email?.toLowerCase().includes(lsSearchTerm.toLowerCase());
                                  const matchesStatus = lsFilterStatus === 'all' || r.status === lsFilterStatus;
                                  return matchesSearch && matchesStatus;
                                })
                                .map((reg) => (
                                <tr key={reg.id} className="hover:bg-[#1c1c38]">
                                  <td className="px-6 py-4 whitespace-nowrap text-sm">{reg.fullName}</td>
                                  <td className="px-6 py-4 whitespace-nowrap text-sm">{reg.email}</td>
                                  <td className="px-6 py-4 whitespace-nowrap text-sm">{reg.phone}</td>
                                  <td className="px-6 py-4 text-sm max-w-xs truncate">{reg.selectedSession}</td>
                                  <td className="px-6 py-4 whitespace-nowrap text-sm">{reg.studentLevel}</td>
                                  ...
                                  <td className="px-6 py-4 whitespace-nowrap">
                                    <select
                                      ...
                                      className={`px-2 py-1 text-xs font-semibold rounded-full ${
                                        reg.status === 'confirmed' ? 'bg-green-900 text-green-200' :
                                        reg.status === 'contacted' ? 'bg-blue-900 text-blue-200' :
                                        reg.status === 'cancelled' ? 'bg-red-900 text-red-200' :
                                  ...
                                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                                    <button
                                      onClick={() => {
                                        alert(`Name: ${reg.fullName}\nEmail: ${reg.email}\nPhone: ${reg.phone}\nSession: ${reg.selectedSession}\nLevel: ${reg.studentLevel}\nGoals: ${reg.goals || 'N/A'}\nNotes: ${reg.additionalNotes || 'N/A'}`);
                                      }}
                    ...
                    {liveSessionTab === 'sessions' && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {liveSessionForms.map((session) => (
                          <div key={session.id} className="bg-[#141426] p-6 rounded-2xl shadow-lg">
                            <div className="flex justify-between items-start mb-4">
                              ...
                              <div className="flex gap-2">
                                <button
                                  ...
                                  className={`p-2 rounded ${session.isActive ? 'text-green-400' : 'text-gray-500'}`}
                                >
                                  {session.isActive ? <FaToggleOn size={24} /> : <FaToggleOff size={24} />}
                                </button>
                                <button
                            ...
                            <div className="mt-4 pt-4 border-t border-gray-700">
                              <div className="text-xs text-gray-400">
                                {liveSessionRegistrations.filter(r => r.selectedSession === session.title).length} registrations
                              </div>
                            </div>
                          </div>
                        ))}
                        {liveSessionForms.length === 0 && (
                          <div className="col-span-2 text-center py-12 text-gray-400">
                            No sessions created yet. Click "Create Session" to add one.
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    {/* Create Session Tab */}
                    {liveSessionTab === 'create' && (
                      <div className="bg-[#141426] p-6 rounded-2xl shadow-lg max-w-3xl mx-auto">
                        <h3 className="text-xl font-bold mb-6">Create New Live Session</h3>
                        <form onSubmit={handleCreateSession} className="space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              ...
                              <input
                                ...
                                className="w-full px-4 py-2 bg-[#1c1c38] border border-gray-700 rounded-lg"
                          ...
                          <div>
                            ...
                            <textarea
                              ...
                              onChange={(e) => setSessionForm({...sessionForm, instructorBio: e.target.value})}
                              className="w-full px-4 py-2 bg-[#1c1c38] border border-gray-700 rounded-lg"
                              rows="2"
                          ...
                          <div>
                            ...
                            <textarea
                              ...
                              className="w-full px-4 py-2 bg-[#1c1c38] border border-gray-700 rounded-lg"
                              rows="3"
                              placeholder="List of topics (one per line)"
                            />
                          </div>
                          ...
                          <div>
                            ...
                            <textarea
                              ...
                              onChange={(e) => setSessionForm({...sessionForm, prerequisites: e.target.value})}
                              className="w-full px-4 py-2 bg-[#1c1c38] border border-gray-700 rounded-lg"
                              rows="2"
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                      <KpiCard 
                        ...
                        value={advisorRequests.length}
                        color="text-blue-500"
                        icon={FaUser}
                      />
                      <KpiCard 
                        title="Pending" 
                        value={advisorRequests.filter(r => r.status === 'pending').length}
                        color="text-yellow-500"
                        icon={FaClock}
                      />
                      <KpiCard 
                        title="Contacted" 
                        value={advisorRequests.filter(r => r.status === 'contacted').length}
                        color="text-blue-500"
                        icon={FaPhone}
                      />
                      <KpiCard 
                        title="Completed" 
                        value={advisorRequests.filter(r => r.status === 'completed').length}
                        color="text-green-500"
                        icon={FaCalendarCheck}
                      />
                    </div>
                    ...
                    {/* Filters */}
                    <div className="bg-[#141426] p-4 rounded-2xl shadow-lg mb-6">
                        ...
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                            advisorFilter === 'all'
                              ...
                              : 'bg-[#1c1c38] text-gray-400 hover:bg-[#252544]'
                          }`}
                        >
                          All ({advisorRequests.length})
                        </button>
                        <button
                          onClick={() => setAdvisorFilter('pending')}
                          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                            advisorFilter === 'pending'
                              ? 'bg-yellow-600 text-white'
                              : 'bg-[#1c1c38] text-gray-400 hover:bg-[#252544]'
                          }`}
                        >
                          Pending ({advisorRequests.filter(r => r.status === 'pending').length})
                        </button>
                        <button
                          onClick={() => setAdvisorFilter('contacted')}
                          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                            advisorFilter === 'contacted'
                              ? 'bg-blue-600 text-white'
                              : 'bg-[#1c1c38] text-gray-400 hover:bg-[#252544]'
                          }`}
                        >
                          Contacted ({advisorRequests.filter(r => r.status === 'contacted').length})
                        </button>
                        <button
                          onClick={() => setAdvisorFilter('completed')}
                          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                            advisorFilter === 'completed'
                              ? 'bg-green-600 text-white'
                              : 'bg-[#1c1c38] text-gray-400 hover:bg-[#252544]'
                          }`}
                        >
                          Completed ({advisorRequests.filter(r => r.status === 'completed').length})
                        </button>
                      </div>
                    </div>
                    ...
                    {/* Requests Table */}
                    <div className="bg-[#141426] p-6 rounded-2xl shadow-lg">
                      <h3 className="text-lg font-medium mb-4">Advisor Requests</h3>
                      <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-800">
                          <thead>
                            <tr>
                          ...
                          <tbody className="divide-y divide-gray-800">
                            {advisorRequests
                              .filter(req => advisorFilter === 'all' || req.status === advisorFilter)
                              .map((request) => (
                              <tr key={request.id} className="hover:bg-[#1c1c38]">
                                <td className="px-6 py-4 whitespace-nowrap text-sm">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                  <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                    request.status === 'pending' ? 'bg-yellow-900/50 text-yellow-300' :
                                    request.status === 'contacted' ? 'bg-blue-900/50 text-blue-300' :
                            ...
                            {advisorRequests.filter(req => advisorFilter === 'all' || req.status === advisorFilter).length === 0 && (
                              <tr>
                ...
                {activeTab === 'partner' && (
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                      ...
                      <KpiCard 
                        title="Contacted" 
                        value={partnerRequests.filter(r => r.status === 'contacted').length}
                        color="text-blue-500"
                        icon={FaPhone}
                      />
                      <KpiCard 
                        title="Approved" 
                        value={partnerRequests.filter(r => r.status === 'approved').length}
                        color="text-green-500"
                        icon={FaCalendarCheck}
                      />
                    </div>
                    ...
                    <div className="bg-[#141426] p-4 rounded-2xl shadow-lg mb-6">
                      <div className="flex gap-2 flex-wrap">
                        ...
                        <button
                          onClick={() => setPartnerFilter('contacted')}
                          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                            partnerFilter === 'contacted'
                              ? 'bg-blue-600 text-white'
                              : 'bg-[#1c1c38] text-gray-400 hover:bg-[#252544]'
                          }`}
                        >
                          Contacted ({partnerRequests.filter(r => r.status === 'contacted').length})
                        </button>
                        <button
                    ...
                    {/* Requests Table */}
                    <div className="bg-[#141426] p-6 rounded-2xl shadow-lg">
                      <h3 className="text-lg font-medium mb-4">Partnership Requests</h3>
                      <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-800">
                          <thead>
                            <tr>
                              <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Date</th>
                              <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">College</th>
                              <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Contact Person</th>
                          ...
                          <tbody className="divide-y divide-gray-800">
                            {partnerRequests
                              .filter(req => partnerFilter === 'all' || req.status === partnerFilter)
                              .map((request) => (
                              <tr key={request.id} className="hover:bg-[#1c1c38]">
                                <td className="px-6 py-4 whitespace-nowrap text-sm">
                                ...
                                <td className="px-6 py-4">
                                  <div className="font-medium">{request.collegeName}</div>
                                  {request.website && (
                                    <a href={request.website} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-400 hover:underline">
                                      Visit Website
                                    </a>
                                  )}
                                </td>
                            ...
                            {partnerRequests.filter(req => partnerFilter === 'all' || req.status === partnerFilter).length === 0 && (
                              <tr>
