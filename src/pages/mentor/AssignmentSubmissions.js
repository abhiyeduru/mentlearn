...
export default function MentorAssignmentSubmissions() {
  ...
  useEffect(() => {
    applyFilters();
  }, [searchTerm, statusFilter, courseFilter, assignmentFilter, studentFilter, submissions]);

  const fetchAllData = async () => {
    try {
      setLoading(true);
      console.log('Fetching assignment submissions for mentor:', currentUser.uid);

      // 1. Fetch assigned students for this mentor
      const mentorAssignmentsQuery = query(
        collection(db, 'mentorAssignments'),
        where('mentorId', '==', currentUser.uid)
      );
      const mentorAssignmentsSnapshot = await getDocs(mentorAssignmentsQuery);
      const assignedStudentIds = mentorAssignmentsSnapshot.docs.map(doc => doc.data().studentId);
      ...
      // 3. Fetch all assignments (we'll filter by students later)
      ...
      // Filter assignments that include any of our assigned students
      const relevantAssignments = assignmentsData.filter(assignment => 
        assignment.studentIds && 
        assignment.studentIds.some(studentId => assignedStudentIds.includes(studentId))
      );
      ...
      // Get assignment IDs (limit to 10 for Firestore 'in' query)
      const assignmentIds = relevantAssignments.map(a => a.id).slice(0, 10);

      // 4. Fetch submissions for these assignments
      let allSubmissions = [];
      
      // If we have more than 10 assignments, we need to batch the queries
      for (let i = 0; i < relevantAssignments.length; i += 10) {
        const batchIds = relevantAssignments.slice(i, i + 10).map(a => a.id);
      ...
      // Filter submissions to only include assigned students
      const filteredSubmissions = allSubmissions.filter(submission =>
        assignedStudentIds.includes(submission.studentId)
      );

      console.log('Found submissions:', filteredSubmissions.length);
      ...
      const enrichedSubmissions = filteredSubmissions.map(submission => {
        const assignment = relevantAssignments.find(a => a.id === submission.assignmentId);
  ...
  const applyFilters = () => {
    ...
    if (studentFilter !== 'all') {
      filtered = filtered.filter(sub => sub.studentId === studentFilter);
    }
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900">Student Assignment Submissions</h1>
            <p className="mt-2 text-sm text-gray-600">
              Review and grade submissions from your assigned students
            </p>
          </div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-12 text-center">
              ...
              <p className="text-gray-600 mb-6">
                {submissions.length === 0 
                  ? "Your assigned students haven't submitted any assignments yet."
